...
public class FamilyTree
{
	...
	//
	// "Depth" of a node is the "distace" between that node and the root. The depth of the root is 0. The
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("node with first name does not exist");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("node with second name does not exist");
